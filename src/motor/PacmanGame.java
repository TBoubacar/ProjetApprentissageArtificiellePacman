...
public class PacmanGame extends Game  {
	...
	/**
	 * Fonction qui permet de savoir si une action est légale pour un agent donné.
	 */
	
	public boolean isLegalMove(Agent agent, AgentAction action) {
		if (agent.is_alive() && action != null) {
	...
	/**
	 * Permet de savoir si il y a un agent fantome
	 */
	public boolean isGhostsAt (int x, int y) {
		for(Agent agentFantome : _agentsFantom) {
			if (agentFantome.get_position().getX() == x && agentFantome.get_position().getY() == y) return true;
		}
		return false;
	}
