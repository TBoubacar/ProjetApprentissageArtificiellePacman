...
public class ApproximateQLearningStrategy extends QLearningStrategy{
	...
	public ApproximateQLearningStrategy(double epsilon, double gamma, double alpha, int sizeX, int sizeY) {
		// TODO Auto-generated constructor stub
		super(epsilon, gamma, alpha);
		this.weights = new double[this.nbAction+1];
		for (int i = 0; i < weights.length; i++) {
			this.weights[i] = 0;
		}
		this.initialiseAleatoireWeight();
		this.sizeX = sizeX;
		this.sizeY = sizeY;
	}

	private void initialiseAleatoireWeight() {
	...
	public AgentAction chooseAction(PacmanGame state) {
		...
		if(Math.random() < epsilon) {			
			...
			ArrayList<AgentAction> tabIdAction = new ArrayList<AgentAction>();
			...
			for(int a=0; a < this.nbAction; a++) {		
				...
				tabIdAction.add(actionChoose);
				
				if(qValue > maxQvalue) {
					maxQvalue = qValue;
					actionChoose = new AgentAction(a);
				} else if(qValue  == maxQvalue) {
					actionChoose = tabIdAction.get(random.nextInt(tabIdAction.size()));
				}
				if (tabIdAction.size() == 2)
					tabIdAction.clear();
